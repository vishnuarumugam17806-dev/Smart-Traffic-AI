"""
Trajectory Graph Engine for cross-camera journey correlation.
Constructs topology graphs of cameras and connecting roads, correlates vehicle sightings
across physical checkpoints, detects impossible transit speeds / route anomalies, and reconstructs journey timelines.
"""
...
class TrajectoryGraphEngine:
    ...
    Correlates vehicle observations across different city cameras using plate text,
    vehicle class, physical travel-time feasibility constraints, and road topology.
    """
    ...
    def get_cameras_and_edges(self, db: Session) -> Dict[str, Any]:
        ...
        for cam in cameras:
            lat = cam.intersection.latitude if cam.intersection and cam.intersection.latitude is not None else 13.0604
            lng = cam.intersection.longitude if cam.intersection and cam.intersection.longitude is not None else 80.2496
    ...
    def reconstruct_trajectory(self, db: Session, plate_number: str) -> Optional[Dict[str, Any]]:
        ...
        obs_list = (
            ...
            .filter(PlateObservation.plate_number == plate_clean)
            .order_by(PlateObservation.timestamp.asc())
            .all()
        )

        if not obs_list:
            # Fuzzy plate matching fallback for observations in the last 24 hours
            yesterday = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(hours=24)
            all_obs = db.query(PlateObservation).filter(PlateObservation.timestamp >= yesterday).all()

            matched_obs: List[PlateObservation] = []
            for o in all_obs:
                if levenshtein_distance(o.plate_number, plate_clean) <= 1:
                    matched_obs.append(o)
            obs_list = sorted(matched_obs, key=lambda x: x.timestamp)
        ...
        duration_sec = max(0.0, (last_seen - first_seen).total_seconds())

        timeline: List[Dict[str, Any]] = []
        estimated_distance = 0.0
        route_anomalies_detected: List[Dict[str, Any]] = []
        global_vehicle_id = obs_list[0].global_vehicle_id or "GV-10482"

        # Pre-cache road map to avoid N+1 queries during trajectory reconstruction
        road_map: Dict[Tuple[int, int], Road] = {
            (r.source_camera_id, r.target_camera_id): r
        ...
        for idx, obs in enumerate(obs_list):
            ...
            location = (
                obs.camera.intersection.name
                if (obs.camera and obs.camera.intersection)
                else "City Intersection"
            )
            lat = (
                obs.camera.intersection.latitude
                if (obs.camera and obs.camera.intersection and obs.camera.intersection.latitude is not None)
                else 13.0604
            )
            lng = (
                obs.camera.intersection.longitude
                if (obs.camera and obs.camera.intersection and obs.camera.intersection.longitude is not None)
                else 80.2496
            )
            ...
            if idx > 0:
                ...
                time_diff = max(0.0, (obs.timestamp - prev_obs.timestamp).total_seconds())
                road = road_map.get((prev_obs.camera_id, obs.camera_id))
                ...
                if road:
                    ...
                    if time_diff > 0:
                        ...
                        if transition_speed > 150.0 or time_diff < (road.expected_travel_time_sec * 0.15):
                            ...
                            reason = (
                                f"Impossible speed of {transition_speed:.1f} km/h detected between "
                                f"{prev_cam_label} and {curr_cam_label}."
                            )
    ...
    def correlate_vehicle(
        ...
        speed_kmh = 0.0
        anomaly: Optional[RouteAnomaly] = None

        if prev_obs:
            time_diff = max(0.0, (timestamp - prev_obs.timestamp).total_seconds())
            if time_diff > 0:
                road = (
                    db.query(Road)
                    .filter(
                ...
                if speed_kmh > 150.0:
                    is_anomaly = True
                    prev_label = prev_obs.camera.name if prev_obs.camera else f"CAM-{prev_obs.camera_id}"
                    reason = f"Physically impossible journey speed ({speed_kmh:.1f} km/h) from {prev_label} to current camera."
                ...
                if is_anomaly:
                    ...
                    location = (
                        curr_camera.intersection.name
                        if (curr_camera and curr_camera.intersection)
                    ...
                    anomaly = RouteAnomaly(
                        ...
                        confidence=0.95,
                        observed_route=(
                            f"{prev_obs.camera.name if prev_obs.camera else f'CAM-{prev_obs.camera_id}'} -> "
                            f"{curr_camera.name if curr_camera else f'CAM-{camera_id}'}"
                        ),
                    ...
                    logger.warning(
                        "ROUTE ANOMALY DETECTED: %s (%s) speed=%.1f km/h",
                        plate_clean,
                        global_id,
                        speed_kmh,
                    )
